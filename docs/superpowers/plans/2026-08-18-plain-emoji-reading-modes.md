...
| `src/layouts/BlogPost.astro`（変更） | Props に `slug` / `mode` / `hasAlternate` を追加し、`<hr />` の直後に切替 UI を差す。`BaseHead` に canonical / alternate を渡す |
| `src/components/BaseHead.astro`（変更） | `canonicalPath` / `alternatePath` の任意 prop を追加 |
| `src/content.config.ts`（変更） | `defaultMode: z.enum(['plain','emoji']).default('plain')` を追加 |
...
Run: `npm test`
Expected: PASS（既存の `mojiemoji.test.ts` 22 件 + 新規 11 件）
...
- Produces:
  ...
  - `BlogPost.astro` の Props に `slug: string` / `mode: ReadingMode` / `hasAlternate: boolean` が増える
...
import { hasMoji, modeRoutes, type ReadingMode } from '../../lib/readingMode';

export async function getStaticPaths() {
	const posts = await getCollection('blog');
	return posts.flatMap((post) => {
		const routes = modeRoutes(post.id, post.data.defaultMode, hasMoji(post.body));
		return routes.map((route) => ({
			params: { slug: route.slug },
			props: { post, mode: route.mode, hasAlternate: routes.length > 1 },
		}));
	});
}
...
const { post, mode, hasAlternate } = Astro.props;
const { Content } = await render(post);
---

<BlogPost {...post.data} slug={post.id} mode={mode} hasAlternate={hasAlternate}>
	<Content components={{ Moji: mode === 'emoji' ? Moji : MojiPlain }} />
</BlogPost>
```
...
以降のテンプレートはこの時点では変更しない（`defaultMode` / `slug` / `mode` / `hasAlternate` は Task 3・4 で使う）。
...
```
dist/blog/cloudflare-skill-five-landmines/emoji/index.html
dist/blog/cloudflare-skill-five-landmines/index.html
dist/blog/gh-img/emoji/index.html
dist/blog/gh-img/index.html
dist/blog/hello/index.html
dist/blog/index.html
dist/blog/macos-app-debug-utm-vm/emoji/index.html
dist/blog/macos-app-debug-utm-vm/index.html
dist/blog/mojiemoji/index.html
dist/blog/mojiemoji/plain/index.html
```

`hello` に副ページが無いこと、`mojiemoji` だけ `/plain/` であることを確認する。
...
for f in dist/blog/cloudflare-skill-five-landmines/index.html \
  ...
  printf '%-56s %s\n' "$f" "$(grep -o 'mojiemoji.jozo.beer' "$f" | wc -l)"
done
```
...
**Interfaces:**
- Consumes: `modeHref` / `ReadingMode`（Task 1）、`BlogPost` の `slug` / `mode` / `defaultMode` / `hasAlternate`（Task 2）
...
grep -c 'aria-label="表示モード"' dist/blog/hello/index.html
```
...
**Interfaces:**
- Consumes: `modeHref` / `alternateMode`（Task 1）、`BlogPost` の `slug` / `mode` / `defaultMode` / `hasAlternate`（Task 2）
...
```astro
{alternatePath && <link rel="alternate" href={new URL(alternatePath, Astro.site)} />}
```
...
```astro
const canonicalPath = modeHref(slug, defaultMode, defaultMode);
const alternatePath = hasAlternate
	? modeHref(slug, alternateMode(mode), defaultMode)
	: undefined;
```
...
echo "--- rel=alternate ---"
grep -o '<link rel="alternate" href="[^"]*"' dist/blog/gh-img/index.html
```
...
Expected: 8 件のみ。`/emoji/` と `/plain/` を含む URL が 1 つも無いこと。
...
Expected: 5 件。リンクはすべて `/blog/<slug>/` 形式で、`/emoji/` や `/plain/` を含まない。
...
**3. Type consistency:** `ReadingMode` / `ModeRoute` / `modeRoutes` / `modeHref` / `alternateMode` / `hasMoji` は Task 1 の定義と Task 2・3・4 の呼び出しで一致。`BlogPost` の追加 Props（`slug` / `mode` / `hasAlternate`）は Task 2 で定義し Task 3・4 で消費。`defaultMode` は `post.data` 経由で `BlogPost` に届く（Task 2 でスキーマ追加済み）。
