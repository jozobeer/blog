...
- **color（明るめパレット・6 桁 hex）**: `mojiemoji-github` の推奨から **Tailwind 400–500 のみ**採用
  （淡い 300 は白背景で読めないため除外）。**`FORBIDDEN_COLORS`（600+：`dc2626` / `16a34a` / `2563eb` / `ca8a04` 等）は使わない**。名前色も不可。
