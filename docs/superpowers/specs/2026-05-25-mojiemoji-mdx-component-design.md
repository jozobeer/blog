...
- **color（明るめパレット・6 桁 hex）**: `mojiemoji-github` の推奨（**Tailwind 300–500 を全部**）を採用。
  当初は白背景向けに 400–500 へ絞ったが、**dark テーマ固定（`<html data-theme="dark">`）に切替えたため淡い 300 系も復帰**。
...
- **inline 既定 height = 24**（`mojiemoji-github` でも有効な既定。ユーザー選択）＋ `align=absmiddle`。
...
- `width`/`height` = `size`（既定 32・試行中。正方キャンバスなので等値）。レイアウトシフト防止。
- 行内の縦位置は `src/styles/global.css` の `img[src*="mojiemoji.jozo.beer"]{vertical-align:middle}` で調整（pico 導入後の最小カスタム CSS。`align="absmiddle"` は絶対中央でやや下がるため廃止）。
