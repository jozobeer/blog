...
- **import は不要**を目標とする: 記事描画箇所（`src/pages/blog/[...slug].astro`）で `<Content components={{ Moji }} />`
  ...
  **実装時に検証**し、不可なら各 `.mdx` 冒頭で `import Moji from '../../components/Moji.astro'` の 1 行 import に fallback する。
...
| `src/components/Moji.astro` | 薄いラッパ: `Astro.props.emoji`（テキスト）を受け取り、ビルド内カウンタで出現位置 `index` を採番し、lib を呼んで `<img>` を出力 | 不純（ビルド順依存はここに閉じる） |
